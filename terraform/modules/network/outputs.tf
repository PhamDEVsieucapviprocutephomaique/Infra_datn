output "resource_group_name" {
    value = azurerm_resource_group.main.name
}
output "hub_vnet_name" {
    value = azurerm_virtual_network.hub.name
}
output "spoke_vnet_name" {
    value = azurerm_virtual_network.spoke.name
}
output "hub_spoke" {
    value = azurerm_virtual_network_peering.hub_spoke.name
}
output "spoke_hub" {
    value = azurerm_virtual_network_peering.spoke_hub.name
}
output "system_pool_subnet_name" {
    value = azurerm_subnet.system_pool_subnet.name
}
output "app_pool_subnet_name" {
    value = azurerm_subnet.app_pool_subnet.name
}
output "cron_job_pool_subnet_name" {        
    value = azurerm_subnet.cron_job_pool_subnet.name
}
output "kafka_subnet_name" {
    value = azurerm_subnet.kafka_subnet.name
}
output "database_subnet_name" {
    value = azurerm_subnet.database.name        
}